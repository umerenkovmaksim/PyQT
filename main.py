...
class MyWidget(QMainWindow, Ui_Form):
    def __init__(self):
        ...
        self.displayed = ''
        self.calculator = ''
        self.btn1.clicked.connect(lambda: self.add(1))
        self.btn2.clicked.connect(lambda: self.add(2))
        self.btn3.clicked.connect(lambda: self.add(3))
        self.btn4.clicked.connect(lambda: self.add(4))
        self.btn5.clicked.connect(lambda: self.add(5))
        self.btn6.clicked.connect(lambda: self.add(6))
        self.btn7.clicked.connect(lambda: self.add(7))
        self.btn8.clicked.connect(lambda: self.add(8))
        self.btn9.clicked.connect(lambda: self.add(9))
        self.btn0.clicked.connect(lambda: self.add(0))
        self.btn_dot.clicked.connect(lambda: self.add('.'))
        self.btn_plus.clicked.connect(lambda: self.operation('+'))
        self.btn_minus.clicked.connect(lambda: self.operation('-'))
        self.btn_mult.clicked.connect(lambda: self.operation('*'))
        self.btn_div.clicked.connect(lambda: self.operation('/'))
        self.btn_eq.clicked.connect(self.result)
        self.btn_pow.clicked.connect(lambda: self.hard_operation('^'))
        self.btn_sqrt.clicked.connect(lambda: self.hard_operation('√'))
        self.btn_fact.clicked.connect(self.hard_operation)
        self.btn_clear.clicked.connect(self.clear)

    def add(self, n):
        self.calculator += str(n)
        self.displayed += str(n)
        self.table.display(self.displayed)
    ...
    def operation(self, n):
        if n == '-':
            ...
            self.displayed += '-'
            self.calculator += n
        elif self.calculator[-1] in '1234567890-':
            self.displayed = ''
            self.calculator += n
            self.table.display('')
        else:
            self.displayed = ''
            self.calculator = self.calculator[:-1] + n
            self.table.display('')

    def result(self):
        try:
            a = eval(self.calculator)
            self.table.display(a)
            self.calculator = str(a)
        except:
            self.table.display('Error')

    def hard_operation(self, n):
        try:
            if n == '^':
                self.calculator += '**'
            elif n == '√':
